...
// Настройка подключения к PostgreSQL с поддержкой SSL для внешней базы
const pool = new Pool({
...
function telegramUser(req) { 
  return validateTelegramInitData(req.headers['x-telegram-init-data']); 
}
  ...
  for (let i = 0; i < order.items.length; i++) {
    ...
    try {
      ...
      await telegram('sendMessage', {
        chat_id: chatId,
        text: `⚠️ Фото товара «${esc(item.name)}» не удалось отправить автоматически. Проверьте ссылку на изображение.`
      });
    }
  }
}
...
app.get('/api/products', async (_req, res) => {
  try { 
    const { rows } = await pool.query('SELECT id, category, name, price::float AS price, img, description AS desc FROM family_shop_products ORDER BY created_at ASC'); 
...
app.post('/api/products', requireAdmin, async (req, res) => {
  try {
    const b = req.body || {};
    if (!b.name || !b.img || !b.desc || !b.category || !Number.isFinite(Number(b.price)) || Number(b.price) < 0) {
    ...
    const { rows } = await pool.query(
      'INSERT INTO family_shop_products (id, category, name, price, img, description) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, category, name, price::float AS price, img, description AS desc',
      [id, String(b.category), String(b.name).trim(), Number(b.price), String(b.img).trim(), String(b.desc).trim()]
    );
    res.json({ ok: true, product: rows[0] });
  } catch(e) {
    console.error(e);
    res.status(500).json({ error: 'Не удалось сохранить товар в базе данных.' });
  }
});

app.delete('/api/products/:id', requireAdmin, async (req, res) => {
  try { 
    const r = await pool.query('DELETE FROM family_shop_products WHERE id=$1', [req.params.id]); 
...
app.get('/api/settings', async (_req, res) => {
  try {
    const r = await pool.query("SELECT value FROM family_shop_settings WHERE key='main'");
    res.json({ settings: r.rowCount ? r.rows[0].value : defaultSettings });
  } catch(e) {
...
app.put('/api/settings', requireAdmin, async (req, res) => {
  try {
    ...
    await pool.query(
      "INSERT INTO family_shop_settings(key, value) VALUES('main', $1) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value, updated_at=NOW()",
      [settings]
    );
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    const items = b.items.map(x => {
      ...
      return p ? { id: p.id, name: p.name, price: Number(p.price), img: p.img, qty: Number(x.qty) || 0 }
               : { name: String(x.name || 'Товар'), price: Number(x.price) || 0, img: x.img || '', qty: Number(x.qty) || 0 };
    }).filter(x => x.qty > 0);
    ...
    const order = {
      ...
      telegram_user_id: b.telegram_user_id ? Number(b.telegram_user_id) : null,
      telegram_username: b.telegram_username ? String(b.telegram_username) : null,
      items,
      total
    };

    const saved = await pool.query(
      'INSERT INTO family_shop_orders(name, phone, address, telegram_user_id, telegram_username, total, items) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id',
      [order.name, order.phone, order.address, order.telegram_user_id, order.telegram_username, total, JSON.stringify(items)]
    );
...
app.get('/api/health', async (_req, res) => {
  try {
    const r = await pool.query('SELECT COUNT(*)::int AS count FROM family_shop_products');
    res.json({ ok: true, telegramConfigured: !!BOT_TOKEN, databaseConfigured: !!DATABASE_URL, products: r.rows[0].count });
  } catch(e) {
    res.status(500).json({ ok: false, error: e.message });
  }
});

// Отдача фронтенда (убедитесь, что HTML файл называется index.html)
app.get('*', (_req, res) => res.sendFile(path.join(__dirname, 'index.html')));

(async () => {
  try {
    await initDb();
    app.listen(PORT, () => console.log(`Family Shop running on port ${PORT}`));
  } catch(e) {
