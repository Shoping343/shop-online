...
app.put('/api/products/:id', adminAuth, async (req, res) => {
  ...
  const { category, name, price, img, desc } = req.body || {};
  if (!id.startsWith('custom-')) return res.status(400).json({ error: 'Этот товар нельзя изменить.' });
  if (!['women','men','kids'].includes(category) || !String(name || '').trim() || !Number.isFinite(Number(price)) || Number(price) < 0 || !String(img || '').trim()) {
    return res.status(400).json({ error: 'Заполните категорию, название, цену и фото.' });
  }
  try {
    const { rows } = await pool.query(
      'UPDATE products SET category=$1,name=$2,price=$3,img=$4,description=$5 WHERE id=$6 RETURNING id,category,name,price,img,description AS desc',
      [category, String(name).trim(), Number(price), String(img).trim(), String(desc || '').trim(), id]
    );
...
app.delete('/api/products/:id', adminAuth, async (req, res) => {
  const id = String(req.params.id || '');
  if (!id.startsWith('custom-')) return res.status(400).json({ error: 'Этот товар нельзя удалить.' });
  try {
